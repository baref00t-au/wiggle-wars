...
/**
 * Combines several input sources that manage different players (e.g. keyboard +
 * touch for humans, plus the AI). For each player, a non-zero steering value
 * from any source wins; later sources override earlier ones when both are active.
 */
export class MergedInput implements InputSource {
  ...
  getInputs(state: GameState): Input[] {
    ...
    for (const source of this.sources) {
      for (const { playerId, turn } of source.getInputs(state)) {
